...
from dasapp.models import DoctorReg, LabWorker, PatientReg, PatientReport,Specialization,CustomUser,Appointment,AddPatient,MedicalHistory, Test
...
def DOCTORHOME(request):
    ...
    latest_appointment = Appointment.objects.filter(doctor_id=doctor_reg).first()
    context = {
    ...
    if request.method == "POST":
        ...
        try:
            ...
            PatientReport.objects.create(
                lab_worker=None,  # Will be assigned by lab worker  # The doctor sending the test
                patient=patient,
                test=test,
                doctor_notes=notes,
                status='requested'  # New status for test requests
            )
            
            messages.success(request, "Test request sent to lab successfully!")
            return redirect('doctor_home')
            
        except (PatientReg.DoesNotExist, Test.DoesNotExist):
            messages.error(request, "Invalid patient or test selected")
    
    # Get all tests and active patients
    tests = Test.objects.all()
    patients = PatientReg.objects.filter(admin__is_active=True)
    
    return render(request, 'doc/send_test_details.html', {
        'tests': tests,
        'patients': patients
    })
